...
import { createReview, getService, updateReview } from "@/api_feats/employeeServices/employeeServicesApi";
...
function ClientService({ appointments, reviews, loadClientReviews, client }) {
    ...
    return (
        <div className="border-2 border-slate-300 rounded-3xl sm:min-w-full sm:w-full w-[95%] sm:m-0 m-auto min-h-[25%] p-4 ">
            <div className="flex flex-row gap-4 h-full relative">
                <Skeleton className="flex min-w-[100px] w-[100px] self-center min-h-[100px] h-[100px] rounded-3xl" isLoaded={imageLoaded}>
                    <img className="w-full h-full object-fill absolute rounded-3xl"
                        src={firstService?.image_base64}
                        alt="esta es la imagen"
                        onLoad={() => setImageLoaded(true)} />
                </Skeleton>
                <div className="gap-4 lg:w-[85%] lg:border-none lg:pt-0 pt-4 border-slate-300">
                    <div className="flex flex-col gap-1">
                        ...
                        <text className="text-xl line-clamp-1 text-ellipsis"><b>Profesional: </b>{appointments?.employee?.user?.first_name} {appointments?.employee?.user?.last_name}</text>
                        <text className="text-xl"><b>Total:</b> <b>${appointments.total_price}</b></text>
                        <div className="flex flex-row space-x-2 flex-nowrap items-center justify-between">
                            <div className="flex flex-row gap-2">
                                <text className="text-xl text-nowrap line-clamp-1 text-ellipsis 2xl:flex hidden">{horaAmigable}</text>
                                <text className="text-xl text-nowrap line-clamp-1 text-ellipsis 2xl:flex hidden">{appointments.date}</text>
                            </div>
                            <Button isIconOnly variant="bordered" radius="full" className="border-1 border-slate-500 shadow-sm shadow-slate-500" onPress={handleOpen}>
